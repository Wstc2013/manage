...
class LogMonitorHandler(tornado.websocket.WebSocketHandler):
    clients = []
    threads = []
    ...
    def open(self):
        # 获取监控的path
        self.file_path = self.get_argument('file_path', '')
        logger.debug("Websocket: LogMonitorHandler file_path : %s" % (self.file_path))
        ...
        logger.debug("Websocket: LogMonitorHandler : %s" % (LogMonitorHandler.threads))
        try:
            ...
            LogMonitorHandler.threads.remove(LogMonitorHandler.threads[client_index])
            logger.debug("Websocket: LogMonitorHandler client error: %s" % str(e))

        logger.debug("Websocket: LogMonitorHandler client num: %s, thread num: %s" % (len(LogMonitorHandler.clients),
                                                                            len(LogMonitorHandler.threads)))
...
class Application(tornado.web.Application):
    def __init__(self):
        handlers = [
            ...
            (r'/logmonitor', LogMonitorHandler),
...
def main():
    ...
    tornado_app = tornado.web.Application(
        [
            ...
            (r'/ws/logmonitor', LogMonitorHandler),
            (r'/kill', WebTerminalKillHandler),
